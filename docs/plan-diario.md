...
| 7 | 26/09 | `POST /api/auth/register` (se une a una organización existente por `organizationSlug`, siempre como USER) y `POST /api/auth/login` con JWT (`sub`, `organizationId`, `role`). El hash no sale del módulo `users` (`verifyCredentials`); el login busca el correo con `runAsSystem`; mismo 401 para correo inexistente y contraseña incorrecta, con un hash ficticio para igualar tiempos; contraseña de 8 a 72 **bytes**. `AuthService` **escrito por Claude a pedido de Jhon**, en lugar de la pieza ✍️. | Día 8: guards, roles y contexto de organización desde el token |
